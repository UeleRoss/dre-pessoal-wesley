...
import type { RecurringBill, RecurringBillInstance } from "@/types/database";
...
const Contas = ({ user }: { user: LocalUser }) => {
  ...
  const { data: bills = [], isLoading: isLoadingBills } = useQuery<RecurringBill[]>({
    ...
    queryFn: async () => offlineDb.fetchRecurringBills(user.id),
  });

  const { data: billInstances = [], isLoading: isLoadingInstances } = useQuery<RecurringBillInstance[]>({
    queryKey: ["recurring-bills-instances", user.id, monthReference],
    queryFn: async () => offlineDb.fetchRecurringBillInstances(user.id, monthReference),
  });
  ...
  const createBillMutation = useMutation({
    mutationFn: async (payload: BillFormValues) => {
      ...
      await offlineDb.insertRecurringBill(user.id, {
  ...
  const updateBillMutation = useMutation({
    mutationFn: async (payload: BillFormValues & { id: string }) => {
      ...
      const sanitizedCategory = payload.category.trim();
      const sanitizedBank = formatBankName(payload.bank);
      await offlineDb.updateRecurringBill(user.id, payload.id, {
        name: payload.name.trim(),
        value,
        due_date: Number(payload.due_date),
        category: sanitizedCategory || null,
  ...
  const deleteBillMutation = useMutation({
    mutationFn: async (id: string) => {
      await offlineDb.deleteRecurringBill(user.id, id);
    },
    onSuccess: () => {
  ...
  const togglePaidMutation = useMutation({
    mutationFn: async ({ bill, isPaid }: { bill: RecurringBill; isPaid: boolean }) => {
      await offlineDb.upsertRecurringBillInstance(user.id, bill.id, monthReference, { pago: isPaid });
    },
    onSuccess: () => {
      invalidateQueries();
    },
  ...
  const adjustValueMutation = useMutation({
    mutationFn: async ({ bill, value }: { bill: RecurringBill; value: number }) => {
      await offlineDb.upsertRecurringBillInstance(user.id, bill.id, monthReference, { valor_ajustado: value });
    },
    onSuccess: () => {
